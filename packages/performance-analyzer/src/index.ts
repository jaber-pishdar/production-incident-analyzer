import type { TraceEntry, EndpointPerformance, PerformanceSummary, Bottleneck, Span } from '@pia/shared';

export function parseTraces(input: string): TraceEntry[] {
  const traces: TraceEntry[] = [];
  for (const line of input.split('\n').filter(Boolean)) {
    try { const p = JSON.parse(line); if (p.traceId && p.method && p.endpoint) traces.push(p); } catch { }
  }
  return traces;
}
...
function computeEndpointStats(traces: TraceEntry[]): EndpointPerformance[] {
  ...
  for (const t of traces) {
    ...
    (epMap.get(key) ?? epMap.set(key, []).get(key)!).push(t.durationMs);
  ...
  return Array.from(epMap.entries()).map(([key, durs]) => {
    ...
    return {
      method, endpoint: pathParts.join(' '), totalRequests: total, errorCount: errors,
      errorRate: (errors / total) * 100, p50Ms: percentile(sorted, 50), p95Ms: percentile(sorted, 95),
      p99Ms: percentile(sorted, 99), avgMs: Math.round(avg * 100) / 100, maxMs: Math.max(...durs),
